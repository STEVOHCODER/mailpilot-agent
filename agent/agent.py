...
def _make_redaction_filter():
    import re as _re
    email_re = _re.compile(r"\b[\w.+-]+@[\w.-]+\.\w+\b")
    digits_re = _re.compile(r"(?<!\w)(\+?\d[\d\s-]{6,}\d)(?!\w)")

    def redact(record):
        text = record.getMessage()
        text = email_re.sub("[email]", text)
        text = digits_re.sub(lambda m: m.group(1)[:4] + "***", text)
        record.msg = text
        record.args = ()
        return True
...
def setup_logging(log_file="logs/agent.log", level="INFO"):
    ...
    public_mode = os.environ.get("MAILPILOT_PUBLIC") == "1"
