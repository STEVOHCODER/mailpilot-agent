...
from app.models import (
    User, EmailConnection, WhatsAppConnection,
...
async def poll_connection(db: Session, conn: EmailConnection):
    ...
    for msg in messages:
        ...
        exists = db.query(MessageLog).filter(
            MessageLog.user_id == user.id,
            MessageLog.message_hash == msg_hash
        ).first()
        if exists:
            continue
        ...
        if daily_limit != -1 and today_count >= daily_limit:
            ...
            log = MessageLog(
                ...
                sender=msg.get("from", "")[:250],
                recipient=msg.get("to", "")[:250],
                importance_score=importance,
                status="skipped",
                error_message="Daily limit reached",
                message_hash=msg_hash,
                matched_rules=json.dumps([r.name for r in matched_rules]),
            )
            db.add(log)
            db.commit()
            break
        ...
        if usage:
            ...
            usage.messages_sent_month += 1
            usage.storage_used_mb = round(usage.storage_used_mb + 0.001, 4)
        else:
            db.add(Usage(user_id=user.id, messages_sent_today=1, messages_sent_month=1, storage_used_mb=0.001))
...
def match_rules(msg: dict, rules: List[ForwardingRule]) -> List[ForwardingRule]:
    ...
    for rule in rules:
        ...
        if rule.min_importance is not None:
            if classify_email(msg) < rule.min_importance:
                continue
        matched.append(rule)
